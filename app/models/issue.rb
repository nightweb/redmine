...
class Issue < ActiveRecord::Base
  ...
  has_many :visible_journals,
    -> { where("(#{Journal.table_name}.private_notes = ? OR (#{Project.allowed_to_condition(User.current, :view_private_notes)}))", false).readonly },
    :class_name => 'Journal',
    :as => :journalized

  has_many :time_entries, :dependent => :destroy
  has_and_belongs_to_many :changesets,
                          -> { order("#{Changeset.table_name}.committed_on ASC").
                                 order("#{Changeset.table_name}.id ASC") }
